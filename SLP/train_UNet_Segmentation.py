...
from keras.layers import Input, Activation, Conv2D, MaxPooling2D, UpSampling2D, concatenate
...
def train_UNet_Segmentation(input_channels, output_channel):
    ##################################
    # General parameters
    ##################################
    batch_size = 8
    ...
    filename_base = datetime.datetime.today().strftime('%Y%m%d') + '_SLP_SubcellularSegmentationLoss_10xWeights_Input_' + \
                    filename_input_channels + 'Predicting_' + channel_names[output_channel[0]]
    ...
    decoded = Conv2D(2, (3, 3), activation='relu', padding='same', name='Conv2D_Decode_Layer1_3')(decode_1)
    softmax = Activation('softmax')(decoded)

    autoencoder = Model(input_img, softmax)
    # sgd = optimizers.SGD(lr = 0.005, decay = 0.0, momentum = 0.0, nesterov = False)
    autoencoder.compile(optimizer='adam', loss=weighted_crossentropy)  # 'mse', optimizer='adadelta'
